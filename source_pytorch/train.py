...
def _get_train_data_loader(batch_size, training_dir):
    ...
    print(training_dir)
    normalize_transfer = transforms.Normalize(mean=[0.5,0.5,0.5], std=[0.5,0.5,0.5])
...
def train(n_epochs, loaders, model, optimizer, criterion, use_cuda, save_dir, save_file):
    """
...
if __name__ == '__main__':
    ...
    parser.add_argument('--save_path', type=str, default='model_fruit.pt', metavar='OUT',
                        help='input dimension (default: 1)')
    ...
    model = FruitClassifier(args.class_count).to(device)
    ...
    train(args.epochs, train_loader, model, optimizer, criterion, use_cuda, args.model_dir, args.save_path)

    print("Finished training. Testing now")

    model.load_state_dict(torch.load(os.path.join(args.data_dir, args.save_path)))
